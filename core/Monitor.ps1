...
function Update-UI {
    ...
    if (-not $state) {
        ...
        try {
            ...
            $orchStatus = switch ($task.State) {
                'Running' { ([char]0x25CF + ' Orchestrator running - waiting for state.json...') }
    ...
    try {
        ...
        $orchStatus = if ($isRunning) { ([char]0x25CF + ' Running') } else { ([char]0x25CB + ' Waiting') }
    ...
    try {
        if ($ageSecs -gt 600 -and $orchStatus -ne ([char]0x25CF + ' Running')) {
